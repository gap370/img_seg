...
class VDSRR(nn.Module):
	def __init__(self):
		...
		self.input = nn.Conv3d(in_channels=1, out_channels=32, kernel_size=3, stride=1, padding=1, bias=False)
		self.output = nn.Conv3d(in_channels=32, out_channels=1, kernel_size=3, stride=1, padding=1, bias=False)
		self.relu = nn.ReLU(inplace=True)
